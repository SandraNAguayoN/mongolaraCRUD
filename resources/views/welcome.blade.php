...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>
            ...
            *,:after,:before{
            ...
            .w-5{
                width:1.25rem}.w-8{width:2rem}.w-auto{width:auto
            }
            .grid-cols-1{
            ...
            @media (min-width:640px){
                ...
                .sm\:h-20{height:5rem}.sm\:ml-0{margin-left:0}.sm\:px-6{padding-left:1.5rem;padding-right:1.5rem}.sm\:pt-0{padding-top:0}.sm\:text-left{text-align:left}.sm\:text-right{text-align:right}}@media (min-width:768px){.md\:border-t-0{border-top-width:0}.md\:border-l{border-left-width:1px}.md\:grid-cols-2{grid-template-columns:repeat(2,minmax(0,1fr))}}@media (min-width:1024px){.lg\:px-8{padding-left:2rem;padding-right:2rem}}@media (prefers-color-scheme:dark){.dark\:bg-gray-800{--bg-opacity:1;background-color:#2d3748;background-color:rgba(45,55,72,var(--bg-opacity))}.dark\:bg-gray-900{--bg-opacity:1;background-color:#1a202c;background-color:rgba(26,32,44,var(--bg-opacity))}.dark\:border-gray-700{--border-opacity:1;border-color:#4a5568;border-color:rgba(74,85,104,var(--border-opacity))}.dark\:text-white{--text-opacity:1;color:#fff;color:rgba(255,255,255,var(--text-opacity))}.dark\:text-gray-400{--text-opacity:1;color:#cbd5e0;color:rgba(203,213,224,var(--text-opacity))}}
        </style>
    ...
    <body class="antialiased" >
        <div style="background: #FFFDE4;
        background: -webkit-linear-gradient(top left, #FFFDE4, #94FCD4);
        background: -moz-linear-gradient(top left, #FFFDE4, #94FCD4);
        background: linear-gradient(to bottom right, #FFFDE4, #94FCD4);"
        class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center sm:pt-0">
            @if (Route::has('login'))
                <div class="hidden fixed top-0 right-0 px-6 py-4 sm:block">
                    @auth
                        <a href="{{ url('/home') }}" class="text-sm text-gray-700" style="font-size:20px; color:black; box-shadow: 10px 10px black; background:white; padding:10px;">Inicio</a>
                    @else
                        <!--<a href="{{ route('login') }}" class="text-sm text-gray-700 underline" style="color:whitesmoke;">Iniciar sesión</a>-->
                        <button style="font-size:20px; color:black; box-shadow: 10px 10px black; background:white; padding:10px;"><a href="{{ route('login') }}" >Iniciar sesión</a></button>

                        @if (Route::has('register'))
                            <!--<a href="{{ route('register') }}" class="ml-4 text-sm text-gray-700 underline" style="color:whitesmoke;" >Registrarse</a>-->
                            <button style="font-size:20px; color:black; box-shadow: 10px 10px black; background:white; padding:10px;"><a href="{{ route('register') }}" >Registrarse</a></button>
                        @endif
                    @endauth
                </div>
            @endif
            ...
            <div>
                <h1 style="color:whitesmoke; text-shadow: 2px 2px 5px black,0 0 25px #7AC1BB, 0 0 5px #62AA77;">Bienvenido(a)</h1>
